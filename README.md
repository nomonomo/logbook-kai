...
**現状はWindows、macOS (Intel / Apple Silicon) 対応**

### その１：証明書のインストールが必要です

艦これのHTTPS化に伴い、ブラウザまたはOSに **ルート CA 証明書（`logbook-ca.crt`）** のインストールが必要になります。

プロキシ側は **ルート証明書方式（`logbook-ca.p12`）** を推奨します。詳細な手順については、[ブラウザの設定(必須)](how-to-preference.md)を参照してください。
...
**起動方法：**
- **Windows版：** 配布フォルダ内の `launch.bat` をダブルクリックして起動してください。
- **macOS版：** 配布フォルダ内の `launch.command` をダブルクリックして起動してください。
...
2. 展開したフォルダ内の `logbook` フォルダと起動ファイルを、既存の航海日誌フォルダにコピー（上書き）する
   - **Windows版：** `launch.bat`、`update_apply.bat`
   - **macOS版：** `launch.command`、`update_apply.command`
3. 起動ファイルを実行する

**注意：** 設定ファイルやデータファイルはそのまま引き継がれます。

**自動更新機能について：**
航海日誌には自動更新機能が実装されています。

- 設定で更新チェックが有効になっている場合、起動時に自動的に更新の有無をチェックします
- 更新が利用可能な場合、アプリケーション内から更新の準備（ダウンロード）を行うことができます
- 更新の準備が完了すると、次回起動時に更新スクリプトが自動的に実行され、更新が適用されます
  - **Windows版：** `update_apply.bat`
  - **macOS版：** `update_apply.command`
- 更新処理のログは `logs/update.log` に記録されます
- 更新に問題が発生した場合は、`logs/update.log` を確認してください
- 更新前のバージョンは `logbook_old` フォルダにバックアップされます

**ゲームデータ更新について：**
マップ・任務条件・識別札などのゲームデータは、アプリ本体とは別に更新できます。

- 設定で「ゲームデータ更新チェック」が有効な場合、起動時・設定画面を開いた時、および前回チェックから 12 時間経過後に GitHub 上のデータを確認します
- 新しいデータがあれば `config/gamedata/` にダウンロードします。読込は外部と同梱のマニフェスト版を比較し、新しい方を使います（同版なら同梱）
- 手動で同ディレクトリにファイルを置いても差し替え可能です
- データの正本はリポジトリの [`data/`](data/README.md) です（アプリ再リリース不要）
- 任務の編集は `data/quest/src/`、配信は `quests.json`（`logbook-data` モジュールがビルド時に生成）

### その４：バージョン 25.10.1～26.5.1 をお使いだった方へ

証明書の扱いが変更されました。詳細は [ブラウザの設定(必須)](how-to-preference.md) を参照してください。

**主な変更点：**

- **推奨方式**: ルート証明書（`logbook-ca.p12`）を使用し、サーバー証明書は起動時に自動生成
- `[設定] - [通信]` に「使用証明書」（ルート証明書 / サーバ証明書）の選択が追加
- ルート証明書は `[設定] - [通信]` の **「証明書作成」**（ルート証明書ファイル行）から作成
- `create-ca-and-cert.bat` の配布は 25.10.1 以降終了済み

**アップデート後の互換性：**

- 従来どおり `kancolle.p12` を指定している場合は、**従来方式のまま動作**します（設定変更不要）
- **`logbook-ca.p12` がある場合は、ルート証明書方式への移行を推奨**します（`kancolle.p12` の更新が不要になります）
- ブラウザへの **`logbook-ca.crt` インストール** は従来どおり必要です

### その５：戦闘ログの「高度な集計」を廃止しました

JDK に JavaScript エンジンが同梱されなくなったため、戦闘ログ画面の **高度な集計**（ユーザーが JavaScript で集計する機能）を廃止しました。右ペインの列集計と円グラフはそのまま使えます。

手元に `config/logbook.bean.BattleLogScriptCollection.json` が残っていても読み込みません。削除する処理は入れていないので、不要なら手動で消してください。
...
Windows版とmacOS版（Intel / Apple Silicon）を配布しております

### 動作確認環境

動作確認を行った環境については、[動作確認環境](tested-environments.md)を参照してください。
